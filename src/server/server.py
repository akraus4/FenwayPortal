...
def findTableData(table_name):
    ...
    if table_name == "work_user":
        ...
        addObject = {}
        i = 0
        cur.execute('SELECT work_team_member.work_team_member_id, work_team.work_team_id, work_user.work_user_id,work_team_member.committed_hours, work_team.work_team_name, work_user.firstname, work_user.lastname FROM work_team LEFT OUTER JOIN work_team_member ON work_team.work_team_id = work_team_member.work_team_id LEFT OUTER JOIN work_user ON work_user.work_user_id = work_team_member.work_user_id')
        for row in cur.fetchall():
            addObject['work_team_member_id'] = row[0]
            addObject['work_team_id'] = row[1]
            addObject['work_user_id'] = row[2]
            addObject['committed_hours'] = row[3]
...
def editTableDataWTeamMember(wTeamMemberId,wTeamId,wUserId,cHours):
    cur.execute("INSERT INTO work_team_member(work_team_member_id,work_team_id,work_user_id,committed_hours)"
                ...
                + "ON DUPLICATE KEY UPDATE work_team_member_id='" + wTeamMemberId + "', work_team_id='" + wTeamId + "', work_user_id='" + wUserId + "', committed_hours='" + cHours + "'")
    statementExecuted = "True"
    return statementExecuted
