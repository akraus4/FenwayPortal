...
echo "$(date "+[%F %T]") - [ INFO ]  - Starting Sprint Load." | tee -a /opt/metrics/environments/prod/logs/sprint-import.log
dotnet VSTSComms.dll load 2>&1 | tee -a /opt/metrics/environments/prod/logs/sprint-import.log
echo "$(date "+[%F %T]") - [ INFO ]  - Completed Sprint Load." | tee -a /opt/metrics/environments/prod/logs/sprint-import.log
popd